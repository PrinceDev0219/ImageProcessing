...
export default function ScanScreen({ navigation }) {
  ...
  const __takePicture = async () => {
    try {
      ...
      uploadData.append('fileData', { uri: photo.uri, name: filename, type });
      // console.log(uploadData);
      // return
      imageUpload(uploadData).then((res) => {
        console.log("imageupload-------------------" + res.data.filepath);
        setImagePath(axios.defaults.baseURL + "/" + res.data.filepath);
        // setLoading(false);
      });
  ...
  const __scanPhoto = () => {
    ...
    .request(options)
    .then(function (res) {
      let widthRate = capturedImage.width / Dimensions.get("screen").width;
      let heightRate = capturedImage.height / Dimensions.get("screen").height;
          console.log("start====", widthRate, "=====", heightRate);
      if(!res.data.ParsedResults) {
        setLoading(false);
      ...
      if (parsedResults != null) {
        ...
        if (selectArray.length != 0) {
          ...
          console.log("adons---", parsedResults[0])
