...
export default function ScanScreen({ navigation }) {
  ...
  const __takePicture = async () => {
    try {
      ...
      const photo = {
        height: 757,
        uri: "file:///data/user/0/host.exp.exponent/cache/ExperienceData/%2540tylerminegar%252FScanToGosug/ImagePicker/60ef6aea-493d-40a3-b8c3-5a0577913d47.jpg",
        width: 582,
      };
  ...
  const __scanPhoto = () => {
    // setLoading(true);
    let match = /\.(\w+)$/.exec((capturedImage.uri).split('/').pop());
    let type = match ? `image/${match[1]}` : `image`
    console.log(type)
    // return
    // let imageData = new FormData();
    //           imageData.append("fileName", (capturedImage.uri).split('/').pop());
    //           imageData.append("contentType", type);
    //           imageData.append("fileKey", "avatars/" + new Date().getTime().toString());

    //           console.log("====imageData=====", imageData)
    //           imageUpload(imageData).then((res) => {
    //             console.log("imageupload-------------------" + res.data);
    //           });
    //           setLoading(false);
    //           return;
    // const data = new FormData();
    // data.append("url", (capturedImage.uri).split('/').pop());
